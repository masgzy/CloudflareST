...
> - Windows **PowerShell** 只需把下面命令中的 `cfst` 改为 `.\cfst.exe` 即可。  
...
cfst -tp 80 -url http://cdn.cloudflare.steamstatic.com/steam/apps/5952/movie_max.webm

# 如果是非 80 443 的其他端口，那么需要确定你使用的下载测速地址是否支持通过该非标端口访问。

# 也可以针对单个 IP 直接指定端口（不需要 -tp），格式见"单独对一个或多个 IP 测速"章节
./cfst -ip 104.248.145.216:6666
```
...
# 指定端口测速（端口会覆盖全局 -tp 参数，仅对当前 IP 生效）
# IPv4 单个 IP 指定端口 → 直接 IP:端口，不要带 /32
./cfst -ip 104.248.145.216:6666

# IPv4 网段指定端口 → 网段:端口
./cfst -ip 1.1.1.0/24:6666

# 混合指定（未指定端口的 IP 使用 -tp 全局端口）
./cfst -ip 1.1.1.1:8080,2.2.2.2/24,2606:4700::/32

# IPv6 指定端口 → 必须用 [] 包裹地址
./cfst -ip [2606:4700::/32]:443
```

> **端口指定规则**：
> - **单个 IPv4**：`IP:端口`，例如 `1.1.1.1:443`，**不要**带 `/32`
> - **IPv4 网段**：`网段:端口`，例如 `1.1.1.0/24:443`
> - **单个 IPv6**：`[IPv6地址]:端口`，例如 `[::1]:443`
> - **IPv6 网段**：`[IPv6网段]:端口`，例如 `[2606:4700::/32]:443`
> - 不指定端口时使用全局 `-tp` 参数（默认 443）
...
> **时区说明**：程序所有时间输出默认使用**系统本地时区**，无需额外配置。
